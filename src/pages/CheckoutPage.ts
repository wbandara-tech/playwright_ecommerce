...
/**
 * CheckoutPage - Represents the checkout page with address and order review.
 */
export class CheckoutPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    // Delivery address detail locators
    ...
    // Billing address detail locators
  ...
  /** Verify delivery address contains expected text */
  async verifyDeliveryAddress(expectedData: {
    name?: string;
    address?: string;
    cityStateZip?: string;
    country?: string;
    ...
    if (expectedData.name) {
      await expect(this.deliveryFirstLastName).toContainText(expectedData.name);
    }
    if (expectedData.address) {
      await expect(this.deliveryAddress1).toContainText(expectedData.address);
    }
    if (expectedData.cityStateZip) {
      await expect(this.deliveryCityStateZip).toContainText(expectedData.cityStateZip);
    }
    if (expectedData.country) {
      await expect(this.deliveryCountry).toContainText(expectedData.country);
    }
    if (expectedData.phone) {
      await expect(this.deliveryPhone).toContainText(expectedData.phone);
    }
  }

  /** Verify billing address contains expected text */
  async verifyBillingAddress(expectedData: {
    name?: string;
    address?: string;
    cityStateZip?: string;
    country?: string;
    ...
    if (expectedData.name) {
      await expect(this.billingFirstLastName).toContainText(expectedData.name);
    }
    if (expectedData.address) {
      await expect(this.billingAddress1).toContainText(expectedData.address);
    }
    if (expectedData.cityStateZip) {
      await expect(this.billingCityStateZip).toContainText(expectedData.cityStateZip);
    }
    if (expectedData.country) {
      await expect(this.billingCountry).toContainText(expectedData.country);
    }
    if (expectedData.phone) {
      await expect(this.billingPhone).toContainText(expectedData.phone);
    }
  }
}
